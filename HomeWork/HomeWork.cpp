...
int main() {
    ...
    // Буфер для всего ввода (до точки)
    ...
    // Фаза 1: ТОЛЬКО ввод, сохранение всего в буфер
    while (true) {
        char c = std::cin.get();
        ...
        if (c == '.') {
            std::cout.put('.');
            std::cout.put('\n');
            // Добавляем точку в буфер как маркер конца
            if (input_pos < 9999) {
        ...
        // Английские буквы - сохраняем в буфер
        if (c >= 'a' && c <= 'z') {
            std::cout.put(c);
            if (input_pos < 9999) {
    ...
    // Фаза 2: Парсинг буфера на слова (уже после ввода точки)
    char words[1000][100];
    int lengths[1000];
    int word_count = 0;
    int pos_in_word = 0;
    ...
    for (int i = 0; i < input_pos; ++i) {
        ...
        if (c == '.') {
            // Точка - конец текста
            if (in_word) {
                lengths[word_count - 1] = pos_in_word;
            }
            break;
        }
        ...
        if (c >= 'a' && c <= 'z') {
            ...
            if (pos_in_word < 99) {
                words[word_count - 1][pos_in_word] = c;
                ++pos_in_word;
            }
        }
        else if (c == ' ') {
    ...
    // Фаза 3: Обработка и вывод результата
