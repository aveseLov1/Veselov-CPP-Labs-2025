...
// Сравнение файлов с информативным выводом позиции при ошибке
bool compare_files(const char* file1, const char* file2) {
    ...
    if (identical) {
        ...
        if (count1 != count2) {
            ...
            std::cerr << "  " << file1 << ": " << (pos + count1) << " байт\n";
            std::cerr << "  " << file2 << ": " << (pos + count2) << " байт\n";
            std::cerr << "Различие начинается с позиции: " << error_pos << " байт\n";
        } else if (count1 > 0) {
            if (std::memcmp(buf1, buf2, count1) != 0) {
                identical = false;
                error_pos = pos;
                // Найдем точную позицию различия в остатке
                for (std::streamsize i = 0; i < count1; ++i) {
                    if (buf1[i] != buf2[i]) {
                        error_pos = pos + i;
                        break;
                    }
    ...
    if (!identical) {
        // Получаем коды символов для лучшей диагностики
        unsigned char ch1 = 0, ch2 = 0;

        // Попробуем прочитать символы в позиции ошибки
        f1.clear();
        f2.clear();
        f1.seekg(error_pos, std::ios::beg);
        f2.seekg(error_pos, std::ios::beg);

        if (f1.good() && f2.good()) {
            ch1 = static_cast<unsigned char>(f1.get());
            ch2 = static_cast<unsigned char>(f2.get());
        }

        std::cerr << "\nФайлы различаются начиная с позиции: " << error_pos << " байт\n";
        std::cerr << "  В файле '" << file1 << "': ";
        if (ch1 == '\n') {
            std::cerr << "символ перевода строки (\\n, код " << static_cast<int>(ch1) << ")";
        } else if (ch1 == '\t') {
            std::cerr << "символ табуляции (\\t, код " << static_cast<int>(ch1) << ")";
        } else if (ch1 == '\r') {
            std::cerr << "символ возврата каретки (\\r, код " << static_cast<int>(ch1) << ")";
        } else if (std::isprint(ch1)) {
            std::cerr << "символ '" << static_cast<char>(ch1) << "' (код " << static_cast<int>(ch1) << ")";
        } else {
            std::cerr << "непечатаемый символ (код " << static_cast<int>(ch1) << ")";
        }

        std::cerr << "\n  В файле '" << file2 << "': ";
        if (ch2 == '\n') {
            std::cerr << "символ перевода строки (\\n, код " << static_cast<int>(ch2) << ")";
        } else if (ch2 == '\t') {
            std::cerr << "символ табуляции (\\t, код " << static_cast<int>(ch2) << ")";
        } else if (ch2 == '\r') {
            std::cerr << "символ возврата каретки (\\r, код " << static_cast<int>(ch2) << ")";
        } else if (std::isprint(ch2)) {
            std::cerr << "символ '" << static_cast<char>(ch2) << "' (код " << static_cast<int>(ch2) << ")";
        } else {
            std::cerr << "непечатаемый символ (код " << static_cast<int>(ch2) << ")";
        }
        std::cerr << "\n";
    }
