    ...
    while (running) {
        ...
        switch (method) {
            ...
            case SolutionMethod::BISECTION: {
                ...
                if (equation_type == EquationType::COS_4X_MINUS_HALF_X) {
                    // std::cout << "Рекомендуемый интервал: [0.2, 0.5]\n";
                    // std::cout << "f(0.2) = cos(0.8) - 0.1 = 0.697 - 0.1 = 0.597 (положительное)\n";
                    // std::cout << "f(0.5) = cos(2.0) - 0.25 = -0.416 - 0.25 = -0.666 (отрицательное)\n";
                }
...
//                 if (equation_type == EquationType::COS_4X_MINUS_HALF_X) {
//                     std::cout << "Рекомендуемый интервал: [1.6, 1.65]\n";  // ← ИЗМЕНИТЬ!
//                 }
...
//     // ИСПРАВЛЕННАЯ ПРОВЕРКА С УЧЕТОМ МАШИННОЙ ТОЧНОСТИ
//     const double machine_epsilon = std::numeric_limits<double>::epsilon() * 10;

//     if (std::abs(f_left) < machine_epsilon) {
//         // Левая граница уже является корнем
//         result.root = left_bound;
//         result.iterations = 0;
//         result.success = true;
//         return result;
//     }

//     if (std::abs(f_right) < machine_epsilon) {
//         // Правая граница уже является корнем
//         result.root = right_bound;
//         result.iterations = 0;
//         result.success = true;
//         return result;
//     }

//     // Проверка, что знаки РАЗНЫЕ (с учетом погрешности)
//     if (f_left * f_right > 0) {
//         result.success = false;
//         result.error_message = "Функция на концах интервала имеет одинаковые знаки";
//         return result;
...
//         // Проверка, не нашли ли точный корень
//         if (std::abs(f_mid) < machine_epsilon) {
//             left = right = mid;
//             break;
//         }
