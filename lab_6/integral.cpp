...
void printTabl(resultToPrint* i_prn, int countRowOfTable, double eps) {
    int widthOfTableColumns[numberOfTableColumns] = {firstColumnWidth, secondColumnWidth, thirdColumnWidth, fourthColumnWidth};
    ...
    for (int j = 0; j < numberOfTableColumns; ++j) {
        for (int k = 0; k < widthOfTableColumns[j] + 2; ++k)
            std::cout << "─";
    ...
    for (int j = 0; j < numberOfTableColumns; ++j) {
        ...
        std::cout << std::string(leftSpaces, ' ') << title[j] << std::string(rightSpaces - 1, ' ');
    }
    ...
    for (int i = 0; i < countRowOfTable; ++i) {
        ...
        std::cout << std::string(func_left, ' ') << i_prn[i].name << std::string(func_right - 1, ' ') << vt;
        ...
        std::cout << " " << std::right << std::setw(widthOfTableColumns[1] + 1) << exact_str << vt;
        ...
        std::cout << " " << std::right << std::setw(widthOfTableColumns[2] + 1) << sum_str << vt;

        // Столбец 4: Количество разбиений
        std::cout << " " << std::right << std::setw(widthOfTableColumns[3] + 1) << i_prn[i].n << vt;
