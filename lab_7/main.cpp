...
int main() {
    ...
    try {
        // NOLINTNEXTLINE(cppcoreguidelines-avoid-magic-numbers, hicpp-avoid-magic-numbers)
        twoDimensionalDynamicMatrix = MatrixModule::allocateMatrix(static_cast<std::size_t>(numberOfRows), static_cast<std::size_t>(numberOfColumns));
    ...
    // Заполнение матрицы
    MatrixModule::fillMatrix(twoDimensionalDynamicMatrix, static_cast<std::size_t>(numberOfRows), static_cast<std::size_t>(numberOfColumns));

    // Вывод матрицы в разных форматах
    std::cout << "\nМатрица в фиксированном формате (точность 4):\n";
    MatrixModule::printDynamicMatrix(twoDimensionalDynamicMatrix, static_cast<std::size_t>(numberOfRows), static_cast<std::size_t>(numberOfColumns),
                                     4, false, 80);
    ...
    const int STATIC_SIZE = 10;  // NOLINT(cppcoreguidelines-avoid-magic-numbers, readability-magic-numbers)
    // NOLINTNEXTLINE(cppcoreguidelines-avoid-c-arrays, hicpp-avoid-c-arrays, modernize-avoid-c-arrays)
    double staticMatrix[STATIC_SIZE][STATIC_SIZE];

    for (int i = 0; i < STATIC_SIZE; ++i) {
        for (int j = 0; j < STATIC_SIZE; ++j) {
            staticMatrix[i][j] = static_cast<double>(i * 10 + j);
        }
    }
    ...
    MatrixModule::printStaticMatrix(pointerArray, static_cast<std::size_t>(STATIC_SIZE), static_cast<std::size_t>(STATIC_SIZE), 2, false, 80);
    ...
    auto B = staticMatrix;  // NOLINT(modernize-use-auto)
    ...
    MatrixModule::freeMatrix(twoDimensionalDynamicMatrix, static_cast<std::size_t>(numberOfRows));
