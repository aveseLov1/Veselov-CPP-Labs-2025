...
void printDynamicMatrix(double** matrix, int numberOfRows, int numberOfColumns, int precision = 6, bool scientificFormat = false,
                        int maxLineLength = 80);

// Функция вывода статической матрицы через вспомогательный массив указателей
void printStaticMatrix(double* rows[], int numberOfRows, int numberOfColumns, int precision = 6, bool scientificFormat = false,
                       int maxLineLength = 80);
}  // namespace MatrixModule
