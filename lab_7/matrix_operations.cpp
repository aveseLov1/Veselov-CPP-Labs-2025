...
void fillMatrix(double **matrix, std::size_t numberOfRows,
                std::size_t numberOfColumns, double x) {
  for (std::size_t i = 0; i < numberOfRows; ++i) {
    for (std::size_t j = 0; j < numberOfColumns; ++j) {
      if (i == j) {
        ...
        double denominator =
            std::pow(static_cast<double>(fact), static_cast<int>(i) + 1);
...
void printDynamicMatrix(const double *const *matrix, std::size_t numberOfRows,
                        std::size_t numberOfColumns, int precision,
                        bool scientificFormat, int maxLineLength) {
  ...
  for (std::size_t startCol = 0; startCol < numberOfColumns;
       startCol += static_cast<std::size_t>(colsPerLine)) {
    std::size_t endCol = std::min(
        startCol + static_cast<std::size_t>(colsPerLine), numberOfColumns);
...
void printStaticMatrix(const double *const rows[], std::size_t numberOfRows,
                       std::size_t numberOfColumns, int precision,
                       bool scientificFormat, int maxLineLength) {
